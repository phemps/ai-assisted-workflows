...
describe('User API Integration', () => {
  ...
  describe('POST /api/users', () => {
    ...
    it('should enforce unique email constraint', async () => {
      ...
      const userCount = await db('users').where({ email: existingUser.email }).count('* as count');
    ...
    it('should handle database transaction rollback', async () => {
      ...
      vi.spyOn(emailService, 'sendWelcomeEmail').mockRejectedValue(new Error('Email service down'));
...
test.describe('Critical User Journeys', () => {
  ...
  test('Complete user registration and onboarding flow', async ({ page }) => {
    ...
    const verificationLink = await getVerificationLinkFromTestEmail('newuser@example.com');
    await page.goto(verificationLink);
    
    // Onboarding flow
    await expect(page.locator('text=Welcome! Let\'s set up your profile')).toBeVisible();
  ...
  test('Complete purchase flow for returning customer', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="order-row"]:first-child')).toBeVisible();
  });
});
```
