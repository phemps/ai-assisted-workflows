...
class FrontendPerformanceAnalyzer:
    ...
    def analyze_frontend_performance(self, target_path: str, min_severity: str = "low") -> Dict[str, Any]:
        """Analyze frontend performance in the target path."""
        
        start_time = time.time()
        result = ResultFormatter.create_performance_result("analyze_frontend.py", target_path)
        ...
        try:
            ...
            for root, dirs, files in os.walk(target_path):
                ...
                for file in files:
                    if self._should_analyze_file(file):
                        ...
                        try:
                            file_findings = self._analyze_file_frontend_performance(file_path, relative_path)
                            file_count += 1
                            ...
                            for finding_data in file_findings:
                                finding = ResultFormatter.create_finding(
                                    ...
                                    title=finding_data['issue_type'].replace('_', ' ').title(),
                                    description=finding_data['message'],
                                    severity=finding_data['severity'],
                                    file_path=finding_data['file'],
                                    line_number=finding_data['line'],
                                    recommendation=self._get_frontend_recommendation(finding_data['issue_type']),
                                    evidence={
                                        'context': finding_data.get('context', ''),
                                        'category': finding_data.get('category', 'frontend_performance'),
                                        'performance_impact': finding_data.get('performance_impact', 'unknown')
                                    }
                                )
            ...
            analysis_summary = self._generate_frontend_summary(performance_summary, file_count)
    ...
    def _analyze_file_frontend_performance(self, file_path: str, relative_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if file_ext in ['.js', '.jsx', '.ts', '.tsx']:
                findings.extend(self._check_performance_patterns(
                    content, lines, relative_path, self.bundle_patterns, 'bundle', 'bundle_size'
                ))
                
                # Check React patterns for React files
                if 'react' in content.lower() or file_ext in ['.jsx', '.tsx']:
                    findings.extend(self._check_performance_patterns(
                        content, lines, relative_path, self.react_patterns, 'react', 'render_performance'
                    ))
                
                # Check JavaScript patterns
                findings.extend(self._check_performance_patterns(
                    content, lines, relative_path, self.js_patterns, 'javascript', 'runtime_performance'
                ))
            
            # Check CSS patterns for CSS files
            elif file_ext in ['.css', '.scss', '.sass', '.less']:
                findings.extend(self._check_performance_patterns(
                    content, lines, relative_path, self.css_patterns, 'css', 'render_performance'
                ))
            
            # Check asset patterns for HTML files
            elif file_ext in ['.html', '.htm']:
                findings.extend(self._check_performance_patterns(
                    content, lines, relative_path, self.asset_patterns, 'assets', 'load_performance'
                ))
    ...
    def _check_performance_patterns(self, content: str, lines: List[str], file_path: str,
                                   pattern_dict: Dict, category: str, performance_impact: str) -> List[Dict[str, Any]]:
    ...
    def _get_frontend_recommendation(self, issue_type: str) -> str:
        ...
        return recommendations.get(issue_type, 'Review for frontend performance optimization opportunities')

    def _generate_frontend_summary(self, performance_summary: Dict, file_count: int) -> Dict[str, Any]:
        """Generate summary of frontend performance analysis."""
        
        # Categorize issues by performance impact
        impact_categories = {
            'bundle_size': [k for k in performance_summary.keys() if k.startswith('bundle_')],
            'render_performance': [k for k in performance_summary.keys() if k.startswith(('react_', 'css_'))],
            'runtime_performance': [k for k in performance_summary.keys() if k.startswith('javascript_')],
            'load_performance': [k for k in performance_summary.keys() if k.startswith('assets_') or 'large_file_size' in k]
        }
        ...
        return {
            ...
            'issues_by_performance_impact': {
                category: {
                    ...
                    'issues': {issue.replace(f'{category}_', ''): performance_summary.get(issue, 0) 
                             for issue in issues if performance_summary.get(issue, 0) > 0}
                }
                for category, issues in impact_categories.items()
            },
            'severity_breakdown': severity_counts,
            'performance_score': self._calculate_performance_score(total_issues, file_count),
            'optimization_opportunities': self._get_optimization_opportunities(performance_summary),
            'recommendations': self._generate_priority_recommendations(performance_summary)
        }
    ...
    def _count_by_severity(self, performance_summary: Dict) -> Dict[str, int]:
        ...
        severity_mapping = {
            ...
            'medium': ['large_imports', 'dynamic_imports_missing', 'missing_memo', 'missing_key_prop', 'unnecessary_rerenders', 'expensive_selectors', 'inefficient_dom_queries', 'large_data_processing', 'missing_lazy_loading', 'large_bundle_assets', 'large_file_size'],
            'low': ['unused_imports', 'inline_object_creation', 'large_css_files', 'unused_css', 'unoptimized_images', 'file_error']
        }
    ...
    def _get_optimization_opportunities(self, performance_summary: Dict) -> List[Dict[str, Any]]:
        """Get the top frontend optimization opportunities."""
        high_impact_patterns = ['large_imports', 'synchronous_operations', 'memory_leaks', 'missing_memo', 'large_bundle_assets']
        opportunities = []
        ...
        for issue, count in performance_summary.items():
            ...
            if issue_name in high_impact_patterns and count > 0:
                opportunities.append({
                    ...
                    'count': count,
                    'impact': 'high' if issue_name in ['synchronous_operations', 'memory_leaks'] else 'medium',
                    'category': issue.split('_')[0]
                })
        
        return sorted(opportunities, key=lambda x: x['count'], reverse=True)[:5]

    def _generate_priority_recommendations(self, performance_summary: Dict) -> List[str]:
        ...
        if any('synchronous_operations' in k for k in performance_summary.keys()):
            recommendations.append("HIGH: Fix synchronous operations blocking the UI thread")
        if any('memory_leaks' in k for k in performance_summary.keys()):
            recommendations.append("HIGH: Clean up memory leaks from timers and event listeners")
        
        # Bundle optimization
        if any('large_imports' in k for k in performance_summary.keys()):
            recommendations.append("MEDIUM: Optimize bundle size with tree shaking and selective imports")
        if any('dynamic_imports_missing' in k for k in performance_summary.keys()):
            recommendations.append("MEDIUM: Implement code splitting for better load performance")
        
        # React optimization
        if any('missing_memo' in k or 'unnecessary_rerenders' in k for k in performance_summary.keys()):
            recommendations.append("MEDIUM: Optimize React rendering with memoization")
        
        # General recommendations
        total_issues = sum(performance_summary.values())
        if total_issues > 15:
            recommendations.append("Consider implementing performance monitoring and metrics")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze frontend performance in codebase')
