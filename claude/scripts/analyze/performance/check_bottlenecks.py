...
class BottleneckAnalyzer:
    ...
    def analyze_bottlenecks(self, target_path: str, min_severity: str = "low") -> Dict[str, Any]:
        """Analyze performance bottlenecks in the target path."""
        
        start_time = time.time()
        result = ResultFormatter.create_performance_result("check_bottlenecks.py", target_path)
        ...
        try:
            ...
            for root, dirs, files in os.walk(target_path):
                ...
                for file in files:
                    if self._should_analyze_file(file):
                        ...
                        try:
                            file_findings = self._analyze_file_bottlenecks(file_path, relative_path)
                            file_count += 1
                            ...
                            for finding_data in file_findings:
                                finding = ResultFormatter.create_finding(
                                    ...
                                    title=finding_data['bottleneck_type'].replace('_', ' ').title(),
                                    description=finding_data['message'],
                                    severity=finding_data['severity'],
                                    file_path=finding_data['file'],
                                    line_number=finding_data['line'],
                                    recommendation=self._get_bottleneck_recommendation(finding_data['bottleneck_type']),
                                    evidence={
                                        'context': finding_data.get('context', ''),
                                        'category': finding_data.get('category', 'performance_bottleneck'),
            ...
            analysis_summary = self._generate_bottleneck_summary(bottleneck_summary, file_count)
    ...
    def _analyze_file_bottlenecks(self, file_path: str, relative_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            findings.extend(self._check_bottleneck_patterns(
                content, lines, relative_path, self.cpu_patterns, 'cpu', 'cpu_performance'
            ))
            
            # Check memory bottleneck patterns
            findings.extend(self._check_bottleneck_patterns(
                content, lines, relative_path, self.memory_patterns, 'memory', 'memory_usage'
            ))
            
            # Check algorithm complexity patterns
            findings.extend(self._check_bottleneck_patterns(
                content, lines, relative_path, self.algorithm_patterns, 'algorithm', 'algorithm_efficiency'
            ))
            
            # Check database bottleneck patterns
            findings.extend(self._check_bottleneck_patterns(
                content, lines, relative_path, self.database_patterns, 'database', 'database_performance'
            ))
            
            # Check concurrency bottleneck patterns
            findings.extend(self._check_bottleneck_patterns(
                content, lines, relative_path, self.concurrency_patterns, 'concurrency', 'concurrency_performance'
            ))
            
            # Additional Python-specific analysis
            if file_path.endswith('.py'):
                findings.extend(self._analyze_python_bottlenecks(content, lines, relative_path))
    ...
    def _check_bottleneck_patterns(self, content: str, lines: List[str], file_path: str,
    ...
    def _analyze_python_bottlenecks(self, content: str, lines: List[str], file_path: str) -> List[Dict[str, Any]]:
                    ...
                    for child in ast.walk(node):
                        if isinstance(child, ast.Global):
                            findings.append({
                                ...
                                'context': lines[loop_line - 1].strip() if loop_line <= len(lines) else "",
                                'category': 'python',
    ...
    def _get_bottleneck_recommendation(self, bottleneck_type: str) -> str:
        ...
        return recommendations.get(bottleneck_type, 'Review code for performance optimization opportunities')

    def _generate_bottleneck_summary(self, bottleneck_summary: Dict, file_count: int) -> Dict[str, Any]:
        ...
        categories = {
            ...
            'algorithm': [k for k in bottleneck_summary.keys() if k.startswith('algorithm_')],
            'database': [k for k in bottleneck_summary.keys() if k.startswith('database_')],
            'concurrency': [k for k in bottleneck_summary.keys() if k.startswith('concurrency_')],
        ...
        return {
            ...
            'bottlenecks_by_category': {
                category: {
                    'count': sum(bottleneck_summary.get(bottleneck, 0) for bottleneck in bottlenecks),
                    'bottlenecks': {bottleneck.replace(f'{category}_', ''): bottleneck_summary.get(bottleneck, 0) 
                                  for bottleneck in bottlenecks if bottleneck_summary.get(bottleneck, 0) > 0}
                }
            ...
            'recommendations': self._generate_priority_recommendations(bottleneck_summary)
        }
    ...
    def _count_by_severity(self, bottleneck_summary: Dict) -> Dict[str, int]:
        ...
        severity_mapping = {
            ...
            'high': ['inefficient_loops', 'expensive_operations_in_loops', 'blocking_io_operations', 'quadratic_complexity', 'race_conditions', 'deadlock_risk'],
            'medium': ['redundant_computations', 'large_object_creation', 'inefficient_data_structures', 'string_concatenation_loops', 'inefficient_search', 'repeated_sorting', 'missing_pagination', 'inefficient_queries', 'thread_pool_exhaustion', 'global_in_loop'],
    ...
    def _get_critical_bottlenecks(self, bottleneck_summary: Dict) -> List[Dict[str, Any]]:
        """Get critical bottlenecks requiring immediate attention."""
        critical_patterns = ['memory_leaks', 'n_plus_one_queries', 'inefficient_loops', 'blocking_io_operations']
        ...
        for bottleneck, count in bottleneck_summary.items():
            ...
            if bottleneck_name in critical_patterns and count > 0:
                critical_bottlenecks.append({
                    ...
                    'count': count,
                    'severity': 'critical' if bottleneck_name in ['memory_leaks', 'n_plus_one_queries'] else 'high',
    ...
    def _generate_priority_recommendations(self, bottleneck_summary: Dict) -> List[str]:
        ...
        if any('memory_leaks' in k for k in bottleneck_summary.keys()):
            recommendations.append("CRITICAL: Fix memory leaks to prevent application crashes")
        if any('n_plus_one_queries' in k for k in bottleneck_summary.keys()):
            recommendations.append("CRITICAL: Optimize N+1 query problems with batch loading")
        ...
        if any('blocking_io_operations' in k for k in bottleneck_summary.keys()):
            recommendations.append("HIGH: Convert blocking I/O to asynchronous operations")
        if any('quadratic_complexity' in k for k in bottleneck_summary.keys()):
            recommendations.append("HIGH: Replace O(n²) algorithms with more efficient alternatives")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze performance bottlenecks in codebase')
