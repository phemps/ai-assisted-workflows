...
class PerformanceBaseliner:
    ...
    def __init__(self):
        ...
        self.perf_commands = {
            'python': {
                'test_runner': ['python', '-m', 'pytest', '--benchmark-only', '--json-report'],
    ...
    def analyze_project_structure(self, target_path: Path) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(target_path):
            ...
            for file in files:
                ...
                try:
                    ...
                    structure_metrics['file_type_distribution'][file_path.suffix.lower()] += 1
        ...
        structure_metrics['largest_files'] = sorted(
            structure_metrics['largest_files'], 
            key=lambda x: x['size'], 
            reverse=True
        )[:10]
    ...
    def measure_build_performance(self, target_path: Path, languages: List[str]) -> Dict[str, Any]:
        ...
        for lang in languages:
            ...
            build_commands = {
                'python': [['python', '-m', 'py_compile'], ['pip', 'install', '-e', '.']],
                'javascript': [['npm', 'install'], ['npm', 'run', 'build']],
                'typescript': [['npm', 'install'], ['npm', 'run', 'build'], ['tsc', '--noEmit']],
            ...
            if lang in build_commands:
                for cmd in build_commands[lang]:
                    cmd_name = ' '.join(cmd)
                    build_results[cmd_name] = self._measure_command_performance(cmd, target_path)
    ...
    def _measure_command_performance(self, command: List[str], cwd: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': 'Command timed out', 'execution_time': 300}
    ...
    def analyze_dependencies(self, target_path: Path) -> Dict[str, Any]:
        ...
        for dep_file, language in dependency_files.items():
            if '*' in dep_file:
                ...
                if matches:
                    found_dependencies[language] = [str(m.relative_to(target_path)) for m in matches]
            else:
                ...
                if dep_path.exists():
                    try:
                        ...
                        found_dependencies[language] = {'file': dep_file, 'error': 'Could not read'}
        
        return found_dependencies

    def generate_performance_recommendations(self, baseline_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def analyze(self, target_path: str) -> AnalysisResult:
        ...
        try:
            ...
            if not target.exists():
                result = self.formatter.create_performance_result("performance_baseline.py", target_path)
            ...
            baseline_data = {
                ...
                'build_performance': self.measure_build_performance(target, list(languages.keys())),
                'timestamp': time.time(),
                'analysis_duration': 0
            }
    ...
    def _should_skip_directory(self, directory: str) -> bool:
        ...
        skip_dirs = {
            ...
            'venv', 'env', '.env', 'vendor', 'logs', 'target'
        }
