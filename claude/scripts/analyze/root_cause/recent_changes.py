...
class ChangeAnalyzer:
    ...
    def __init__(self, days_back: int = 30, max_commits: int = 100):
        ...
        self.change_patterns = {
            ...
            "config_changes": {
                "patterns": [r'config', r'settings', r'environment', r'\.env', r'constants'],
                "severity": "medium",
                "description": "Configuration changes"
            },
            "dependency_changes": {
                "patterns": [r'package\.json', r'requirements\.txt', r'Gemfile', r'pom\.xml', r'Cargo\.toml'],
                "severity": "medium",
                "description": "Dependency changes"
            },
            "critical_file_changes": {
                "patterns": [r'main\.', r'app\.', r'index\.', r'server\.', r'__init__\.'],
                "severity": "high",
                "description": "Critical application file changes"
            }
        }
    
    def run_git_command(self, command: List[str], cwd: Optional[Path] = None) -> Optional[str]:
        ...
        for line in output.strip().split('\n'):
            if '|' in line:
                ...
                if len(parts) >= 4:
                    ...
                    files_command = ['git', 'show', '--name-only', '--pretty=format:', parts[0]]
                    files_output = self.run_git_command(files_command, repo_path)
                    if files_output:
                        commit_data['files_changed'] = [f.strip() for f in files_output.split('\n') if f.strip()]
            ...
            if line.startswith('\t'):
                ...
                if current_commit:
                    ...
                    if 'author-time' in current_commit:
                        commit_date = datetime.fromtimestamp(int(current_commit['author-time']))
                        if commit_date > datetime.now() - timedelta(days=self.days_back):
    ...
    def analyze_change_risk(self, commits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_risk_level(self, risks: List[str]) -> str:
        ...
        critical_risks = ['hotfix', 'rollback', 'temp_fix', 'auth_changes', 'database_changes']
    ...
    def get_changed_files(self, repo_path: Path) -> List[Dict[str, Any]]:
        ...
        command = [
            'git', 'log', '--name-status', '--since', since_date,
    ...
    def analyze_file_change_frequency(self, changed_files: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    def analyze_commit_timing_patterns(self, commits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, commit in enumerate(commits):
            try:
                ...
                if i > 0:
                    prev_commit_time = datetime.fromisoformat(commits[i-1]['date'].replace(' ', 'T').split('+')[0])
...
def main():
    ...
    result = AnalysisResult(AnalysisType.ARCHITECTURE, "recent_changes.py", str(repo_path))
    ...
    if not commits:
        ...
        for i, timing_issue in enumerate(timing_issues):
            ...
            description = f"Commit {commit['hash'][:8]} made during {concern.replace('_', ' ')}"
            ...
            finding_obj = Finding(
                ...
                title=f"Suspicious Commit Timing",
                ...
                evidence={
                    ...
                    'risk_level': risk_level,
                    **{k: v for k, v in timing_issue.items() if k not in ['commit', 'timing_concern', 'risk_level']}
                }
            )
            result.add_finding(finding_obj)
    ...
    for i, hotspot in enumerate(hotspots):
        finding_obj = Finding(
            ...
            severity=Severity.HIGH if hotspot['risk_level'] == 'high' else Severity.MEDIUM,
    ...
    metadata = {
        ...
        "risky_commits": len([r for r in risky_commits if r['risk_level'] in ['critical', 'high']]) if 'risky_commits' in locals() else 0,
