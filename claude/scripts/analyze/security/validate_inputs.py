...
class InputValidationAnalyzer:
    ...
    def analyze_input_validation(self, target_path: str, min_severity: str = "low") -> Dict[str, Any]:
        """Analyze input validation vulnerabilities in the target path."""
        
        start_time = time.time()
        result = ResultFormatter.create_security_result("validate_inputs.py", target_path)
        ...
        try:
            ...
            for root, dirs, files in os.walk(target_path):
                ...
                for file in files:
                    if self._should_analyze_file(file):
                        ...
                        try:
                            file_findings = self._analyze_file_input_validation(file_path, relative_path)
                            file_count += 1
                            ...
                            for finding_data in file_findings:
                                finding = ResultFormatter.create_finding(
                                    ...
                                    recommendation=self._get_input_validation_recommendation(finding_data['vuln_type']),
                                    evidence={
                                        'context': finding_data.get('context', ''),
                                        'category': finding_data.get('category', 'input_validation'),
            ...
            analysis_summary = self._generate_input_validation_summary(vulnerability_summary, file_count)
    ...
    def _analyze_file_input_validation(self, file_path: str, relative_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.validation_patterns, 'validation', 'input_validation'
            ))
            
            # Check SQL injection patterns
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.sql_injection_patterns, 'sql_injection', 'sql_injection'
            ))
            
            # Check NoSQL injection patterns
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.nosql_injection_patterns, 'nosql_injection', 'nosql_injection'
            ))
            
            # Check path traversal patterns
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.path_traversal_patterns, 'path_traversal', 'path_traversal'
            ))
            
            # Check command injection patterns
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.command_injection_patterns, 'command_injection', 'command_injection'
            ))
            
            # Check LDAP injection patterns
            findings.extend(self._check_input_patterns(
                content, lines, relative_path, self.ldap_injection_patterns, 'ldap_injection', 'ldap_injection'
            ))
    ...
    def _check_input_patterns(self, content: str, lines: List[str], file_path: str,
                             pattern_dict: Dict, category: str, injection_type: str) -> List[Dict[str, Any]]:
    ...
    def _get_input_validation_recommendation(self, vuln_type: str) -> str:
        ...
        return recommendations.get(vuln_type, 'Implement proper input validation and sanitization')

    def _generate_input_validation_summary(self, vulnerability_summary: Dict, file_count: int) -> Dict[str, Any]:
        """Generate summary of input validation analysis."""
        
        # Categorize vulnerabilities by injection type
        injection_categories = {
            'input_validation': [k for k in vulnerability_summary.keys() if k.startswith('validation_')],
            'sql_injection': [k for k in vulnerability_summary.keys() if k.startswith('sql_injection_')],
            'nosql_injection': [k for k in vulnerability_summary.keys() if k.startswith('nosql_injection_')],
            'path_traversal': [k for k in vulnerability_summary.keys() if k.startswith('path_traversal_')],
            'command_injection': [k for k in vulnerability_summary.keys() if k.startswith('command_injection_')],
            'ldap_injection': [k for k in vulnerability_summary.keys() if k.startswith('ldap_injection_')]
        }
        ...
        return {
            ...
            'issues_by_injection_type': {
                category: {
                    ...
                    'vulnerabilities': {vuln.replace(f'{category}_', ''): vulnerability_summary.get(vuln, 0) 
                                     for vuln in vulns if vulnerability_summary.get(vuln, 0) > 0}
                }
            ...
            'critical_injection_vectors': self._get_critical_injection_vectors(vulnerability_summary),
    ...
    def _count_by_severity(self, vulnerability_summary: Dict) -> Dict[str, int]:
        ...
        severity_mapping = {
            'critical': ['direct_user_input', 'string_concatenation', 'format_string_sql', 'os_command_injection', 'shell_metacharacters'],
            'high': ['missing_input_validation', 'dynamic_query_building', 'mongodb_injection', 'json_injection', 'directory_traversal', 'file_inclusion', 'ldap_filter_injection'],
    ...
    def _get_critical_injection_vectors(self, vulnerability_summary: Dict) -> List[Dict[str, Any]]:
        """Get critical injection vectors requiring immediate attention."""
        critical_patterns = ['direct_user_input', 'string_concatenation', 'format_string_sql', 'os_command_injection', 'shell_metacharacters']
        critical_vectors = []
    ...
    def _generate_priority_recommendations(self, vulnerability_summary: Dict) -> List[str]:
        ...
        if any('direct_user_input' in k for k in vulnerability_summary.keys()):
            recommendations.append("CRITICAL: Never directly execute user input - implement safe alternatives")
        if any('string_concatenation' in k or 'format_string_sql' in k for k in vulnerability_summary.keys()):
            recommendations.append("CRITICAL: Replace SQL string concatenation with parameterized queries")
        if any('os_command_injection' in k or 'shell_metacharacters' in k for k in vulnerability_summary.keys()):
            recommendations.append("CRITICAL: Fix command injection by avoiding system calls with user input")
        
        # High priority validation issues
        if any('missing_input_validation' in k for k in vulnerability_summary.keys()):
            recommendations.append("HIGH: Implement comprehensive input validation for all user inputs")
        if any('directory_traversal' in k or 'file_inclusion' in k for k in vulnerability_summary.keys()):
            recommendations.append("HIGH: Add path validation to prevent directory traversal attacks")
        
        # General recommendations
        total_issues = sum(vulnerability_summary.values())
        if total_issues > 10:
            recommendations.append("Consider implementing a centralized input validation framework")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze input validation vulnerabilities in codebase')
