#!/usr/bin/env python3
"""
Platform detection header for SuperCopilot analysis scripts.
...
def detect_python_command():
    ...
    print("SuperCopilot scriptable analysis requires Python 3.8+", file=sys.stderr)
    print("", file=sys.stderr)
    ...
    if platform_name == "Windows":
        ...
        print("  3. Use package manager: 'winget install Python.Python.3'", file=sys.stderr)
    elif platform_name == "Darwin":  # macOS
        print("macOS installation options:", file=sys.stderr)
        print("  1. Install Xcode Command Line Tools: 'xcode-select --install'", file=sys.stderr)
        ...
        print("  1. Ubuntu/Debian: 'sudo apt update && sudo apt install python3'", file=sys.stderr)
...
def print_usage_info(script_name):
    ...
    print(f"# SuperCopilot Analysis Script: {script_name}")
    print(f"# Platform: {platform_name}")
    print(f"# Python: {python_cmd}")
    print(f"#")
    
    if platform_name == "Windows":
        print(f"# Windows Usage:")
        print(f"#   {python_cmd} {script_name} [directory]")
        print(f"#   python {script_name} [directory]")
    else:
        print(f"# macOS/Linux Usage:")
        print(f"#   {python_cmd} {script_name} [directory]")
...
def validate_python_version(min_version=(3, 8)):
    ...
    if current_version < min_version:
        print(f"CRITICAL ERROR: Python version {current_version[0]}.{current_version[1]} is too old", file=sys.stderr)
        print(f"SuperCopilot requires Python {min_version[0]}.{min_version[1]}+", file=sys.stderr)
        print("", file=sys.stderr)
        ...
        platform_name = platform.system()
        if platform_name == "Windows":
            print("  - Download latest from https://python.org/downloads/", file=sys.stderr)
            print("  - Or update via Microsoft Store", file=sys.stderr)
        elif platform_name == "Darwin":  # macOS
            print("  - Use Homebrew: 'brew upgrade python'", file=sys.stderr)
            print("  - Or download latest from https://python.org/downloads/", file=sys.stderr)
        else:  # Linux
...
def ensure_compatible_environment():
    """
    Ensure Python environment is compatible with SuperCopilot.
