...
class LanguageDetector:
    ...
    def detect_from_files(
        cls, file_paths: List[Path], exclusion_patterns: Optional[List[str]] = None
    ) -> Set[str]:
        """
    ...
    def detect_from_directory(
        cls, project_dir: str, exclusion_patterns: Optional[List[str]] = None
    ) -> Set[str]:
        """
        ...
        for pattern, language in cls.LANGUAGE_PATTERNS.items():
    ...
    def get_supported_languages(cls) -> List[str]:
        ...
        return sorted(list(set(cls.LANGUAGE_PATTERNS.values())))
...
def main():
    ...
    if args.output == "json":
        ...
        print(json.dumps({"languages": sorted(list(languages))}, indent=2))
    else:
        print(
