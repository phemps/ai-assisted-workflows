...
def count_expected_vulnerabilities(app_name: str, expected_data: Dict, analyzer: str = None) -> int:
    ...
    if analyzer:
        ...
        print(f"    DEBUG: {analyzer} should detect: {analyzer_types}")
            ...
            if "locations" in obj:
                ...
                if analyzer_types is None:
                    ...
                    if current_key in analyzer_types:
                        print(f"    DEBUG: Counting {current_key} vulnerability (matches analyzer)")
                        return len(obj.get("locations", []))
                    else:
                        print(f"    DEBUG: Skipping {current_key} vulnerability (not for this analyzer)")
                        return 0
            else:
                total = 0
    ...
    print(f"    DEBUG: Total expected vulnerabilities for {app_name} with {analyzer}: {result}")
    return result

def calculate_simplified_metrics(results: Dict, expected_data: Dict, analyzer_name: str) -> List[Dict]:
    ...
    for app, counts in by_app.items():
        counts["expected"] = count_expected_vulnerabilities(app, expected_data, analyzer_name)
...
def run_analyzer_direct(analyzer_script: Path, app_path: Path, app_name: str, max_files: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        env = os.environ.copy()
        pythonpath = str(Path(__file__).parent.parent.parent / "shared")
        env['PYTHONPATH'] = pythonpath
        
        print(f"    DEBUG: Working directory: {os.getcwd()}")
        print(f"    DEBUG: PYTHONPATH: {pythonpath}")
        print(f"    DEBUG: App path exists: {app_path.exists()}")
        print(f"    DEBUG: Analyzer script exists: {analyzer_script.exists()}")
        ...
        # Debug subprocess results
        print(f"    DEBUG: Return code: {result.returncode}")
        print(f"    DEBUG: Stdout length: {len(result.stdout) if result.stdout else 0}")
        print(f"    DEBUG: Stderr preview: {result.stderr[:200] if result.stderr else 'None'}")
        
        # Store raw output for consolidated debug report (don't save individual files)
        ...
        if result.returncode == 0 and result.stdout:
            try:
                ...
                print(f"    DEBUG: JSON structure type: {type(output_data)}")
                if isinstance(output_data, dict):
                    print(f"    DEBUG: JSON keys: {list(output_data.keys())}")
                    findings = output_data.get('findings', [])
                    print(f"    DEBUG: Findings field type: {type(findings)}, length: {len(findings) if isinstance(findings, list) else 'N/A'}")
                    ...
                    print(f"    DEBUG: Direct list with {len(findings)} items")
                print(f"    ✅ Success: {len(findings)} findings in {execution_time:.1f}s")
            ...
            return {"error": "Analyzer execution failed", "returncode": result.returncode, "stderr": result.stderr}
        ...
        return {
            ...
            "success": True,
            "raw_output": result.stdout if result.stdout else "NO OUTPUT"  # Store for debug report
        }
...
def calculate_metrics(app_name: str, analyzer_name: str, findings: List[Dict], expected_findings: Dict) -> Dict[str, Any]:
...
def main():
    import os  # Import here to avoid potential issues
    ...
    parser.add_argument("--config", default="expected_findings.json", help="Expected findings config")
    parser.add_argument("--output", default="results/minimal_evaluation.json", help="Output file")
    parser.add_argument("--max-files", type=int, default=50, help="Max files per analyzer")
    parser.add_argument("--applications", nargs="*", help="Specific applications to test")
    ...
    vulnerable_apps_dir = script_dir.parent / "vulnerable-apps"
    shared_analyzers_dir = project_root / "shared" / "analyzers"
    
    # Find analyzer script
    analyzer_mapping = {
        "detect_secrets": shared_analyzers_dir / "security" / "detect_secrets_analyzer.py",
    ...
    for app_name in applications:
        app_path = vulnerable_apps_dir / app_name
        if not app_path.exists():
            print(f"⚠️  Application not found: {app_path}")
            continue
        
        print(f"\n📂 Evaluating: {app_name}")
        result = run_analyzer_direct(analyzer_script, app_path, app_name, args.max_files)
        
        if result.get("success"):
            # Calculate metrics
            metrics = calculate_metrics(app_name, args.analyzer, result["findings"], expected_findings)
    ...
    valid_metrics = [r["metrics"] for r in results if r.get("success") and "metrics" in r]
    avg_metrics = {}
    if valid_metrics:
        avg_metrics = {
            "precision": sum(m["precision"] for m in valid_metrics) / len(valid_metrics),
        ...
        "detailed_results": [
            {
                ...
                "language": expected_findings.get("applications", {}).get(r.get("application", "unknown"), {}).get("language", "unknown"),
    ...
    # Create consolidated debug report
    debug_report = {
        "evaluation_metadata": {
            "timestamp": evaluation_results["metadata"]["evaluation_date"],
            "analyzer": args.analyzer,
    ...
    for result in results:
        if result.get("success") and "raw_output" in result:
            app_name = result.get("application", "unknown")
            debug_report["applications"][app_name] = {
                "findings_count": result.get("findings_count", 0),
                "execution_time": result.get("execution_time", 0),
                "raw_analyzer_output": result["raw_output"]
            }
    
    debug_output_path = output_path.parent / f"debug_{args.analyzer}_evaluation.json"
    with open(debug_output_path, 'w') as f:
        json.dump(debug_report, f, indent=2)
    
    print(f"🔧 Debug report saved to: {debug_output_path}")
    print(f"\n📊 Evaluation Summary:")  
    ...
    print("This table shows the percentage of expected vulnerabilities detected per application:")
    print()
    
    simplified_metrics = calculate_simplified_metrics(evaluation_results, expected_findings, args.analyzer)
    if simplified_metrics:
        ...
        for metric in simplified_metrics:
            ...
            print(f"  {app_name:<17}  {issues_found:>12}  {issues_expected:>15}  {coverage_percent:>8.1f}%")
    else:
